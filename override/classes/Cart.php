...
class CartCore extends ObjectModel {
    ...
    public function addManequine($autodate = true, $null_values = false) {
        if (!$this->id_lang)
            $this->id_lang = Configuration::get('PS_LANG_DEFAULT');
        if (!$this->id_shop)
            $this->id_shop = Context::getContext()->shop->id;
        ...
        foreach ($result as &$row) {
            ...
            if ($this->_taxCalculationMethod == PS_TAX_EXC) {
                $row['price'] = Product::getPriceStatic(
                                (int) $row['id_product'], false, isset($row['id_product_attribute']) ? (int) $row['id_product_attribute'] : null, 2, null, false, true, (int) $row['cart_quantity'], false, ((int) $this->id_customer ? (int) $this->id_customer : null), (int) $this->id, ((int) $address_id ? (int) $address_id : null), $specific_price_output, true, true, $cart_shop_context
                ); // Here taxes are computed only once the quantity has been applied to the product price

                $row['price_wt'] = Product::getPriceStatic(
                                (int) $row['id_product'], true, isset($row['id_product_attribute']) ? (int) $row['id_product_attribute'] : null, 2, null, false, true, (int) $row['cart_quantity'], false, ((int) $this->id_customer ? (int) $this->id_customer : null), (int) $this->id, ((int) $address_id ? (int) $address_id : null), $null, true, true, $cart_shop_context
                );
                ...
                $row['price'] = Product::getPriceStatic(
                                (int) $row['id_product'], false, (int) $row['id_product_attribute'], 2, null, false, true, $row['cart_quantity'], false, ((int) $this->id_customer ? (int) $this->id_customer : null), (int) $this->id, ((int) $address_id ? (int) $address_id : null), $specific_price_output, true, true, $cart_shop_context
                );

                $row['price_wt'] = Product::getPriceStatic(
                                (int) $row['id_product'], true, (int) $row['id_product_attribute'], 2, null, false, true, $row['cart_quantity'], false, ((int) $this->id_customer ? (int) $this->id_customer : null), (int) $this->id, ((int) $address_id ? (int) $address_id : null), $null, true, true, $cart_shop_context
                );
    ...
    public function containsProductManequine($id_product) {
        $sql = 'SELECT cp.`id_product` FROM `' . _DB_PREFIX_ . 'custom_maneq` cp';
     ...
     */
    public function updateQtyManquine($quantity, $id_product, $id_product_attribute = null, $id_customization = false, $operator = 'up', $id_address_delivery = 0, Shop $shop = null, $auto_add_cart_rule = true) {
        ...
        $result = $this->containsProductManequine($id_product);

        /* Update quantity if product already exist */
        if (!$result) {
            $result_add = Db::getInstance()->insert('custom_maneq', array(
                'id_product' => (int) $id_product,
                'id_product_attribute' => (int) $id_product_attribute,
                'id_cart' => (int) $this->id,
    ...
    public static function getManequineById($id_cart, $idLang, $cover = 1, $frontImage = 1) {
        $sql = 'SELECT `' . _DB_PREFIX_ . 'custom_maneq`.`id_product_attribute`,`' . _DB_PREFIX_ . 'custom_maneq`.`id`,`' . _DB_PREFIX_ . 'custom_maneq`.`id_product`,`path` as front_image,`layer`, `' . _DB_PREFIX_ . 'product_lang`.`name`, `' . _DB_PREFIX_ . 'image`.`id_image`';
        $sql .= 'FROM `' . _DB_PREFIX_ . 'custom_maneq` ';
        $sql .= 'INNER JOIN `' . _DB_PREFIX_ . 'custom_maneq_image` ON `' . _DB_PREFIX_ . 'custom_maneq`.`id_product`=`' . _DB_PREFIX_ . 'custom_maneq_image`.`id_product` ';
        $sql .= 'LEFT JOIN `' . _DB_PREFIX_ . 'product_lang` ON `' . _DB_PREFIX_ . 'custom_maneq`.`id_product` = `' . _DB_PREFIX_ . 'product_lang`.`id_product` ';
        $sql .= 'LEFT JOIN `' . _DB_PREFIX_ . 'image` ON `' . _DB_PREFIX_ . 'custom_maneq`.`id_product` = `' . _DB_PREFIX_ . 'image`.`id_product` ';
        $sql .= 'WHERE `id_cart`=' . (int) $id_cart . '  AND `front_image`=' . $frontImage . '  AND  ';
        $sql .= '`' . _DB_PREFIX_ . 'product_lang`.`id_lang` = ' . (int) $idLang . ' AND ';
        $sql .= '`' . _DB_PREFIX_ . 'image`.`cover` = ' . $cover;
        $result = Db::getInstance()->executeS($sql);
        if ($result) {
            return $result;
        }
    ...
    public static function getManequineBackImage($id) {
        $sql = 'SELECT `path` as `back_image`,`layer` as `back_layer` FROM `ps_custom_maneq_image` WHERE `front_image`= 0 AND `id_product`=' . (int) $id;
        $result = Db::getInstance()->executeS($sql);
        if ($result) {
            return $result;
        }
    ...
    public static function deleteManequineId($id,$card_id) {
        $sql = 'DELETE FROM `ps_custom_maneq` WHERE `id`='.(int)$id.' and `id_cart`='.(int)$card_id;
    ...
    public function duplicate() {
        ...
        foreach ($products as $product) {
            ...
            $success &= $cart->updateQty(
                    $product['quantity'], (int) $product['id_product'], (int) $product['id_product_attribute'], null, 'up', (int) $id_address_delivery, new Shop($cart->id_shop)
            );
        }
